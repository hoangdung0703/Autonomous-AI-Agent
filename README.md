...
1. **System instruction, entity-counting.** Instructed the model to identify how many distinct entities a comparison question names and search separately per entity when the count is 3 or more. This never fired on `q15`, because the question names only two entities — "employees" and "vendors" — even though the vendor side alone requires two documents. The real gap is document count *per side*, not entity count in the question's phrasing, so an entity-counting trigger structurally cannot see it.
2. **System instruction, per-side completeness check.** Instructed the model, after finding a document for each side of a comparison, to explicitly consider whether that side might be governed by a second document and search again if so. This instruction never triggered in practice: the first document found for the vendor side (`vendor-contract-terms.pdf`) looked like a complete, on-topic answer, giving the model no internal signal to doubt it. This attempt also had a measurable side effect — it added one extra "just to be sure" search on ordinary 2-document comparisons that didn't need it (observed on `q10` and `h5`, both otherwise unaffected), showing that strengthening a shared instruction has diffuse costs even when it fails to fix the target case.
3. **Architectural change, gated self-critique step.** Added a separate, single-purpose LLM call outside the main reasoning loop that reviews a candidate answer against the original question before finalizing, gated to fire only when the answer draws on 2+ distinct retrieved documents (so single-document queries pay no added cost). This was reverted before ever reaching a `q15` result, because the very first regression check — `q1`, a genuinely single-document question — unexpectedly triggered it: `q1`'s one search happened to also surface a deliberately-included decoy document (`employee-handbook-2023-archived.pdf`) alongside the real one, so the "2+ distinct documents retrieved" gate fired on retrieval noise rather than genuine multi-document need, violating the explicit requirement that single-document queries stay completely unaffected.

Across all three attempts — two prompt-level, one architectural — the common failure mode is the same: nothing in the retrieved content itself signals incompleteness, so any mechanism that relies on the model (or a critique of the model) noticing its own gap keeps missing this exact case, while loosening the trigger condition enough to catch it reliably (attempt 3) also risks false-triggering on genuinely simple queries. This points to a structural limitation of retrieval-then-reason architectures in general — when a retrieved source looks complete but isn't, reasoning over what was retrieved cannot manufacture the missing signal — rather than a defect specific to this implementation. It is left unfixed and documented rather than patched with a special case; see Future Work below.
...
- **Retrieval breadth on multi-entity comparisons.** The `q15` limitation above is the clearest open item, and three differently-shaped fix attempts (two prompt-level instructions, one architectural self-critique step — see above) all failed to close it without side effects, converging on the same diagnosis: the gap is invisible from the retrieved content itself, so no amount of reasoning over what was already retrieved can reliably surface it. Closing this properly likely requires a retrieval-side fix rather than a reasoning-side one — e.g., for comparison-type questions, always searching a broader candidate pool per side (or per document already found) regardless of how "done" the model feels, rather than depending on the model or a critique of the model to notice the gap on its own.
